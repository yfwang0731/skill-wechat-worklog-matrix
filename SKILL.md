...
- **纯文本值会被表格引擎静默改写，脚本已自动转义**：值一律以 `opType:"formula"` 写入，因此
...
- **本地通道自动读历史**（openpyxl，零额外成本）；**云文档通道默认不读**（见下）。
- **孤儿尾行按预期保留**：若末数据行只在非探针列（如项目编号）有值，它会被**当作有效数据行保留**，
  本批从它下方开始追加（宁留一个空行，也不覆盖）。要复用这类行需显式 `--start-row`。

**云文档拿不到"便宜的历史岗位"**（`download_file` 要登录态、`get_typed_value` 跳过空格、
`get_range_data` 每格带样式、`find_range_data` 的 `filter` 未公开），所以一律走 `--history` 手动喂入。
四条路的逐条实测见 `references/workflow-notes.md` 的「云文档读历史列没有便宜路径」。
...
2. **消息表**：`Msg_<md5(wxid)>` 分布在 `message_1/2/3.db`（`_2` 最老 → `_3` 最新）。
   发送者 id 是**分片内**的 `Name2Id.rowid`（跨分片会变）；`create_time` 为秒级。
3. **消息类型**：`local_type` 取**低 32 位**为类型 —— `1`=文字，`3`=图片，`34`=语音，`43`=视频，
   `47`=表情，`49`=链接/文件，`50`=通话，`57`=引用回复。
4. **正文编码**：`WCDB_CT_message_content` 决定 `message_content` 形态 —— `0`=明文 str，
   `4`=**ZSTD 压缩 bytes**（魔数 `28 B5 2F FD`）。长文本、图片、引用、合并转发、通话基本都走压缩，
   所以导出侧**必须能解压**：解不开时输出**明确标记**（绝不 `errors="replace"` 吐乱码，
   那会把整段会话变成乱码并漏掉需求），并在导出末尾统计未解码条数。
5. **`lt=49` / `lt=57` 的正文常藏在 `<refermsg>` 里**：对方用「引用回复」提需求时，`<title>` 通常只是
...
- **不要用 `AskUserQuestion` 逐条问会话**：会话多时写清单交用户勾选，或按关键字过滤。
- **子代理产出的 JSON 可能带 `null` / 空串字段**：脚本已兜底，但裁决 `merged_preview.csv` 时也要扫一眼。
