...
**结论**：云文档**可以**表内复用，前提是按 `references/kdocs-channel.md`「WPS 通道读表」的第 2 趟把关键列**按行读全**。
...
- **已内置在 `final`**：历史区 = `[表头行+1, 起始行-1]`，复用发生在写 `final_rows.csv`
...
- **独立补跑为什么必须显式说明"本批新行"**：默认模式无从判断 —— 追加起始行 = 末数据行 + 1，
  从那里向下扫恒为空，只会得到 0 条；表格尾部若还有空的"带格式行"，**连提示都不打**（静默无操作）。
...
> **规则本体逐条在 `SKILL.md`「判定规则」**，本节**不再抄一遍** —— 两处各写一份、措辞不同，
