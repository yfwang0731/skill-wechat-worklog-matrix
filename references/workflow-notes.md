...
- 云文档纯文本风险（**真机实测，已自动转义**）：值一律以 `opType:"formula"` 写入，故
  `= + - @ '` 开头 / 前导零 / ≥16 位纯数字的文本会被当公式或数值 —— 实测
  `0012`→12、`=A1`→**被求真值**（变成 A1 的内容）、`12345678901234567`→丢精度、`+86`→86，
  都是**静默改写、不报错**。`to_kdocs_payload.py` 默认给这类值**前置一个单引号**：
  引擎当文本标记消费掉、**回读值不含引号**（无损），并列出命中项；
